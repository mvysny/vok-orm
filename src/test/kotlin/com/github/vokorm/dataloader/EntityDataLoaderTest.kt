...
class EntityDataProviderTest : DynaTest({
    ...
    withAllDatabases {
        ...
        test("sorting") {
            ...
            expect((90 downTo 15).toList()) { ds.getItems(sortBy = listOf("age".desc), range = 0..99).map { it.age } }
        }
        ...
        test("filterTest1") {
            ...
            expect((30..60).toList()) { ds.getItems(sortBy = listOf("age".asc), range = 0..99).map { it.age } }
        }
        ...
        test("filterTest2") {
            ...
            expect((30..60).toList()) { ds.getItems(sortBy = listOf("age".asc), filter = filter, range = 0..99).map { it.age } }
        }

        test("paging") {
            db { for (i in 15..90) Person(name = "test$i", age = i).save() }
            val ds = Person.dataLoader.withFilter { Person::age between 30..60 }
            expect((30..39).toList()) { ds.getItems(sortBy = listOf("age".asc), range = 0..9).map { it.age } }
            expect((40..49).toList()) { ds.getItems(sortBy = listOf("age".asc), range = 10..19).map { it.age } }
        }

        test("nativeQuery") {
            db { for (i in 15..90) Person(name = "test$i", age = i).save() }
            val ds = Person.dataLoader.withFilter { Person::age lt 60 and "age > :age"("age" to 29)}
            expect((30..59).toList()) { ds.getItems().map { it.age } }
        }
    }
})
