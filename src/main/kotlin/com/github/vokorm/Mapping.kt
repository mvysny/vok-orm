...
interface Entity<ID: Any> : Serializable {
    ...
    fun save(validate: Boolean = true) {
        ...
        db {
            if (id == null) {
                ...
                handle.createQuery("update ${meta.databaseTableName} set ${fields.joinToString { "$it = :$it" }} where ${meta.idProperty.dbColumnName} = :${meta.idProperty.dbColumnName}")
                    .bindAliased(this@Entity)
                    .setColumnMappings(meta.getSql2oColumnMappings())
                    .executeUpdate()
                check(handle.result == 1) { "We expected to update only one row but we updated ${handle.result} - perhaps there is no row with id $id?" }
            }
        }
    }
    ...
    fun create(validate: Boolean = true) {
        if (validate) { validate() }
        db {
            var fields: Set<String> = meta.persistedFieldDbNames
            if (id == null) {
                // the ID is auto-generated by the database, do not include it in the INSERT statement.
                fields = fields - meta.idProperty.dbColumnName
            }
            handle.createQuery("insert into ${meta.databaseTableName} (${fields.joinToString()}) values (${fields.joinToString { ":$it" }})", true)
                    .bindAliased(this@Entity)
                    .setColumnMappings(meta.getSql2oColumnMappings())
                    .executeUpdate()
            if (id == null) {
                val key: Any = requireNotNull(handle.key) { "The database have returned null key for the created record. Have you used AUTO INCREMENT or SERIAL for primary key?" }
    ...
    fun delete() {
        ...
        db {
            handle.createQuery("delete from ${meta.databaseTableName} where ${meta.idProperty.dbColumnName} = :id")
                .addParameter("id", id)
                .executeUpdate()
        }
    }
 ...
 * @property entityClass usually a class implementing [Entity] but may be any class.
 */
data class EntityMeta(val entityClass: Class<out Any>) : Serializable {
    ...
    val databaseTableName: String get() {
        val annotatedName: String? = entityClass.getAnnotation<Table>(Table::class.java)?.dbname
    ...
    fun getProperty(propertyName: String): PropertyMeta {
        val result: PropertyMeta? = properties.firstOrNull { it.name == propertyName }
...
private fun Class<*>.findDeclaredField(name: String): Field? {
    ...
    val f: Field? = declaredFields.firstOrNull { it.name == name }
    if (f != null) return f
...
fun Query.bindAliased(entity: Any): Query {
    val meta: EntityMeta = entity.javaClass.entityMeta
