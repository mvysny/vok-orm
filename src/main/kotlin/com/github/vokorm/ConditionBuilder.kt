...
public class ConditionBuilder<T : Any>(public val clazz: Class<T>) {
     ...
     */
    public infix fun <R : Serializable?> KProperty1<T, R>.eq(value: R): Condition = toExpression(clazz).eq(value)

    /**
     * This property value must be less-than or equal to given [value]. Calls [Expression.le].
     */
    public infix fun <R> KProperty1<T, R?>.le(value: R): Condition = toExpression(clazz).le(value)

    /**
     * This property value must be less-than given [value]. Calls [Expression.lt].
     */
    public infix fun <R> KProperty1<T, R?>.lt(value: R): Condition = toExpression(clazz).lt(value)

    /**
     * This property value must be greater-than or equal to given [value]. Calls [Expression.ge].
     */
    public infix fun <R> KProperty1<T, R?>.ge(value: R): Condition = toExpression(clazz).ge(value)

    /**
     * This property value must be greater-than given [value]. Calls [Expression.gt].
     */
    public infix fun <R> KProperty1<T, R?>.gt(value: R): Condition = toExpression(clazz).gt(value)

    /**
     * This property value must not be equal to given [value]. Calls [Expression.ne].
     */
    public infix fun <R> KProperty1<T, R?>.ne(value: R): Condition = toExpression(clazz).ne(value)

    /**
     * This property value must be one of the values
     * provided in the [value] collection. Calls [Expression.in].
     */
    public infix fun <R> KProperty1<T, R>.`in`(value: Collection<R>): Condition = toExpression(clazz).`in`(value)
     ...
     */
    public infix fun KProperty1<T, String?>.like(pattern: String?): Condition = toExpression(clazz).like(pattern)
     ...
     * @return the condition, not null.
     */
    public infix fun KProperty1<T, String?>.likeIgnoreCase(pattern: String?): Condition = toExpression(clazz).likeIgnoreCase(pattern)
    ...
    public infix fun <R : Comparable<R>> KProperty1<T, R?>.between(range: ClosedRange<R>): Condition =
        toExpression(clazz).between(range.start, range.endInclusive)

    /**
     * Matches only when the property is null.
     */
    public val KProperty1<T, *>.isNull: Condition get() = toExpression(clazz).isNull

    /**
     * Matches only when the property is not null.
     */
    public val KProperty1<T, *>.isNotNull: Condition get() = toExpression(clazz).isNotNull

    /**
     * Matches only when the property is true. See [Expression.isTrue] for more details.
     */
    public val KProperty1<T, Boolean?>.isTrue: Condition get() = toExpression(clazz).isTrue

    /**
     * Matches only when the property is false. See [Expression.isFalse] for more details.
     */
    public val KProperty1<T, Boolean?>.isFalse: Condition get() = toExpression(clazz).isFalse
